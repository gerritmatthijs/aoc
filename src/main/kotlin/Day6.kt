...
fun main() {
    ...
    fun List<List<Char>>.rotateLeft() = this.indices.map { i ->
        this[i].indices.map { j -> this[j][n-1-i] }
    }

    fun List<List<Char>>.moveLeft(row: Int, beginCol: Int, endCol: Int) = toMutableList().also {
        it[row] = it[row].subList(0, endCol) + listOf(if (endCol == 0) 'X' else '^') + "X".repeat(beginCol - endCol).toList() + it[row].subList(beginCol + 1, n)
    }
    ...
    val answerPart1 = grid.rotateLeft().play().sumOf { row -> row.count { it == 'X' } }
    println(answerPart1)
